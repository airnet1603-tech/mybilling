...
class TopologiController extends Controller
{
    ...
    public function updateOlt(Request $request, $id)
    {
        ...
        $data = $request->only(['name','ip_address','username','password','snmp_community','hsgq_key','api_endpoint','sync_interval','lat','lng','model','olt_color','olt_icon','odc_color','odc_icon','odp_color','odp_icon','line_olt_odc','line_odc_odp','line_odp_odp']);
        $data['api_endpoint'] = $data['api_endpoint'] ?? '';
        $olt->update($data);
    ...
    public function storeOlt(Request $request)
    {
        ...
        $data2 = $request->only(['name','ip_address','username','password','snmp_community','hsgq_key','api_endpoint','sync_interval','lat','lng','model']);
        $data2['api_endpoint'] = $data2['api_endpoint'] ?? '';
        Olt::create($data2);
...
$onus = Onu::with(['pelanggan.paket', 'odp', 'olt'])->get()->map(fn($o) => [
    ...
    public function syncOnu($olt_id)
    {
        ...
        try {
            ...
            } elseif (in_array($model, ['hioso', 'ha7302cst', 'epon', 'hioso epon', 'hioso epon olt'])) {
                // HiOSO EPON - HA7302CST
                $client  = new \GuzzleHttp\Client(['base_uri' => $baseUri, 'timeout' => 15, 'verify' => false, 'auth' => [$olt->username, $olt->password]]);
                $ponHtml = (string) $client->get('/onuConfigPonList.asp')->getBody();
                preg_match_all("/'([\d\/]+)','ONU Total/", $ponHtml, $ponMatch);
                $ports   = !empty($ponMatch[1]) ? $ponMatch[1] : ['0/1/1', '0/1/2'];
                foreach ($ports as $port) {
                    $res = (string) $client->get('/onuConfigOnuList.asp', ['query' => ['oltponno' => $port]])->getBody();
                    preg_match_all("/'([\d\/\:]+)','([^']*)','([0-9a-fA-F:]+)','(Up|Down)'/", $res, $m);
                    foreach ($m[1] as $i => $onuId) {
                        Onu::updateOrCreate(
                            ['onu_id' => $onuId, 'olt_id' => $olt_id],
                            ['name' => $m[2][$i], 'mac_address' => $m[3][$i], 'status' => $m[4][$i]]
                        );
                        $synced++;
                    }
                }
            ...
            } elseif (in_array($model, ['zte c300', 'zte c320', 'huawei ma5608t', 'huawei ma5800', 'fiberhome', 'nokia'])) {
                return response()->json(['success' => false, 'error' => 'Model '.$olt->model.' belum didukung auto-sync. Silakan hubungi developer.']);
            } else {
                $client = new \GuzzleHttp\Client(['base_uri' => $baseUri, 'timeout' => 15, 'verify' => false, 'auth' => [$olt->username, $olt->password]]);
